...
import {BookOpen,Boxes,Home,ImageIcon,MapPinned,ScanSearch,Settings,Sparkles,Users} from "lucide-react";
import {useProject} from "./project-provider";

const nav=[
  ["/","Cinematic Studio",Home],
  ["/projects","Projects",Boxes],
  ["/novel","Novel Import",ScanSearch],
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 lg:grid lg:grid-cols-[260px_1fr]">
    <aside className="border-b border-slate-200 bg-white p-4 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
      <div className="mb-6 flex items-center gap-3 px-2 py-2">
        ...
        <div><div className="font-black">Fantasy Studio AI</div><div className="text-xs text-slate-500">Cinematic continuity pipeline</div></div>
      </div>
      ...
      <div className="mt-6 hidden rounded-2xl border border-slate-200 bg-slate-50 p-4 lg:block">
        ...
        <div className="mt-1 text-xs text-slate-500">{project.images.length} scenes · {project.characters.length} characters</div>
    ...
    <main className="min-w-0">
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur-xl md:px-7">
        <div><div className="text-sm font-semibold">{project.storyTitle||project.name}</div><div className="text-xs text-slate-500">{project.visualStyle} · {project.aspectRatio} · {project.continuityMode} continuity</div></div>
        <Link href="/" className="text-xs font-semibold text-violet-700">Open Cinematic Studio</Link>
      </header>
