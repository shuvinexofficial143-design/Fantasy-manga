  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
    <aside className="border-b border-white/8 bg-black/20 p-4 backdrop-blur-xl lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
      <div className="mb-6 flex items-center gap-3 px-2 py-2">
        <div className="grid h-10 w-10 place-items-center rounded-2xl bg-violet-500 shadow-lg shadow-violet-500/20"><Sparkles size={20}/></div>
        <div><div className="font-black">Fantasy Studio AI</div><div className="text-xs text-zinc-500">Cinematic image pipeline</div></div>
      </div>
      ...
      <div className="mt-6 hidden rounded-2xl border border-white/8 bg-white/[.03] p-4 lg:block">
        <div className="text-xs uppercase tracking-wider text-zinc-500">Active project</div>
        <div className="mt-2 truncate text-sm font-semibold">{project.name}</div>
        <div className="mt-1 text-xs text-zinc-500">{project.images.length} cinematic images · {project.characters.length} characters</div>
      </div>
    </aside>
    <main className="min-w-0">
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/8 bg-[#08090c]/80 px-4 backdrop-blur-xl md:px-7">
        <div><div className="text-sm font-semibold">{project.storyTitle||project.name}</div><div className="text-xs text-zinc-500">{project.visualStyle} · {project.aspectRatio}</div></div>
        <Link href="/" className="text-xs font-semibold text-violet-300">Open Cinematic Studio</Link>
      </header>
