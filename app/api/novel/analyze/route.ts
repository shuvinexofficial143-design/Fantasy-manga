...
export async function POST(req:Request){
  try{
    ...
    const prompt=[
      ...
      "EXPLAINER STYLE — HARD LOCK: Write an immersive YouTube story recap, NOT a synopsis, book review, formal summary or documentary description. When the chapter follows one clear protagonist, narrate naturally in FIRST PERSON from that protagonist's point of view (मैं/मेरे/मुझे in Hindi; I/my/me in English), unless first person would contradict the source. Make it feel as if the protagonist is telling the audience what happened.",
      "EXPLAINER DETAIL: Do NOT aggressively compress the chapter. Retain the concrete event chain, motivations, relationships, internal thoughts, meaningful reactions, causes, consequences, reveals and useful dialogue. Remove only repetition or prose that adds no spoken-story value. A 1,300–1,500 word source chapter will often need roughly 850–1,200 spoken words when the source supports that detail; this is guidance, never a quota.",
      "EXPLAINER DRAMATIZATION: Prefer moment-to-moment storytelling: event → protagonist observation/reaction → meaningful dialogue/thought → consequence/transition. Do not flatten a conversation into indirect summary when its dialogue reveals personality, stakes, motivation or conflict. Preserve concise direct dialogue in fresh paraphrased wording where useful.",
    ...
    if(mode!=="chunk"&&parts.length>1&&draftExplainer){
      try{
        const polishPrompt=[
          ...
          "HARD STYLE LOCK: Preserve the immersive protagonist-led voice from the drafts. When one clear protagonist drives the chapter, keep FIRST-PERSON narration throughout; never convert it back into third-person synopsis.",
          "Preserve concrete event detail, internal reactions and useful direct dialogue. Do not shorten merely for concision. Remove only duplicated segment introductions/facts and genuinely awkward repetition.",
