...
export async function POST(req:Request){
  try{
    ...
    const prompt=[
      ...
      "POV RULE: Choose POV adaptively from the source. For intimate protagonist-led passages, thoughts, conversations and personal experience, first-person can be strongest. For battles, multi-character action, crowd reactions, world events or cinematic spectacle, close third-person can be stronger. Do not force first-person everywhere and do not switch POV casually inside one continuous scene. Stay character-centric and immersive whichever POV is chosen.",
      ...
      "EXPLAINER DRAMATIZATION: Reconstruct the chapter as spoken cinematic story beats rather than summarizing outcomes. Use the source-supported chain SETUP → ACTION → concrete visual/progression detail → REACTION → useful thought/dialogue/explanation → CONSEQUENCE/NEXT BEAT whenever those elements actually exist. Do not force every element into every moment. For spectacle or danger, reveal information progressively when the source supports it: appearance → threat/action → reaction → payoff. Do not flatten a conversation into indirect summary when dialogue reveals personality, stakes, motivation or conflict. Preserve concise direct dialogue in fresh paraphrased wording where useful.",
      "PROGRESSION RULE: Preserve concrete numbers, distances, counts, levels, timings, rankings, skill effects and before/after comparisons when they establish power, progress, danger, strategy or payoff. Do not replace meaningful measurable progression with vague phrases.",
      "ACTION CHAIN RULE: Never jump straight from an important setup to its result when the source contains visually meaningful intermediate actions. Preserve command/decision → movement or positioning → skill/action → visible effect → target/observer reaction → consequence as separate spoken beats where supported.",
      "VIEWPOINT ACCESS RULE: POV is a storytelling tool, not a hard format. Keep protagonist experience close when useful, but allow brief enemy, ally or crowd reactions/thoughts when the source provides them and they explain danger, surprise, power, strategy or emotional payoff. Do not invent inner thoughts.",
      ...
      "EXPLAINER VOICE: Use easy, spoken, modern Hindi with familiar English terms where they sound natural for Indian YouTube audiences (for example class, level, combat, nervous, school, power). Avoid overly literary Hindi and avoid polished AI-summary phrases. Use short and medium sentences, natural bridges, occasional emphasis and emotional beats. Never add fake hype, facts, jokes, thoughts or dialogue absent from the source.",
    ...
    if(mode!=="chunk"&&parts.length>1&&draftExplainer){
      try{
        const polishPrompt=[
          ...
          "HARD STYLE LOCK: Preserve an immersive character-centric cinematic YouTube storytelling voice from the drafts. Keep the established POV for each coherent passage: intimate protagonist experience may be first-person, while battle/multi-character/crowd spectacle may use close third-person. Never flatten either into distant synopsis, and never switch POV casually.",
          ...
          "Use natural spoken modern Hindi with familiar English terms when the narration is Hindi. Keep short/medium spoken sentences and event → reaction/thought/dialogue → consequence flow. Avoid formal synopsis language, generic AI-summary phrases and editorial narration.",
