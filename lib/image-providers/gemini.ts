...
export async function generateWithGemini(input:ImageGenerationInput):Promise<ImageGenerationResult>{
  ...
  const references=(input.referenceImages||[]).filter(Boolean).slice(0,8),labels=(input.referenceLabels||[]).slice(0,references.length),refParts=await Promise.all(references.map((value,index)=>loadReference(value,index))),negative=input.negativePrompt?`AVOID: ${input.negativePrompt}`:"";
  const referenceMap=references.length?references.map((_,index)=>`Reference ${index+1}: ${labels[index]||`visual reference ${index+1}`}`).join("\n"):"";
