...
export async function generateWithGemini(input:ImageGenerationInput):Promise<ImageGenerationResult>{
  ...
  const prompt=[input.prompt,references.length?`REFERENCE ROLE MAP:\n${referenceMap}\nUse each image only for the role stated above. Character masters define identity/outfit; location masters define architecture; previous generated frame defines immediate pose progression, injuries, props, screen direction and background state.`:"",negative,`Continuity anchor: ${input.seed}. Do not render this number as text.`,"Return exactly ONE image. No captions, speech bubbles, watermark, logo, manga page, webtoon page, comic gutters or split-panel layout."].filter(Boolean).join("\n\n");
